...
int main(void)
{
  ...
  int tempFra = 0;                        // fractional value of temperature (0 to 9375)
	...
	unsigned short Xacc = 0;
	//unsigned short Yacc = 0;
	//unsigned short Zacc = 0;
	
  PLL_Init();
  UART_Init();
  I2C_Init();
	
	// write commands to 0x1D
  I2C_Send2(0x1D,0x16,0x01);		// slave device: 0x1D, register: 0x16, value: 0x05
	Delay(100);
	while(1)
	{
		// set pointer to xout8
		I2C_Send1(0x1D,0x06);					// slave device: 0x1D, register(value): 0x06
		// read from 0x1D to get data
		Xacc = I2C_Recv(0x1D);
